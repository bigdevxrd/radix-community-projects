...
const BOT_COMMANDS = [
  { section: "Getting Started", cmds: [
    ...
    { cmd: "/badge", desc: "Check badge status, tier, XP" },
    { cmd: "/wallet", desc: "Wallet address + badge info" },
    { cmd: "/trust", desc: "Trust score (Bronze/Silver/Gold)" },
    { cmd: "/help", desc: "Full command list" },
  ]},
  { section: "Governance & Voting", cmds: [
    { cmd: "/proposals", desc: "All active proposals" },
    { cmd: "/temps", desc: "Active temp checks only" },
    { cmd: "/decisions", desc: "Link to 47-decision governance tree" },
    { cmd: "/vote <id>", desc: "Vote on a proposal (buttons)" },
    { cmd: "/results <id>", desc: "View vote results" },
    { cmd: "/propose <title>", desc: "Create Yes/No proposal (+25 XP)" },
    { cmd: "/poll <q> | opt1 | opt2", desc: "Multi-choice poll (+25 XP)" },
    { cmd: "/temp <question>", desc: "24h temp check, non-binding (+10 XP)" },
    { cmd: "/charter", desc: "Charter progress (47 decisions)" },
    { cmd: "/charter guide", desc: "Guided voting through charter" },
    { cmd: "/cv2", desc: "On-chain consultations (CV2)" },
    { cmd: "/cv3", desc: "Conviction voting proposals (CV3)" },
    { cmd: "/cv3 status", desc: "CV3 sync + pool balance" },
  ]},
  { section: "Bounties & Tasks", cmds: [
    { cmd: "/bounty", desc: "Guided menu (create, claim, view)" },
    { cmd: "/bounty list", desc: "Open tasks" },
    { cmd: "/bounty create <xrd> <title>", desc: "Create a task (badge required)" },
    { cmd: "/bounty claim <id>", desc: "Claim a funded task" },
    { cmd: "/bounty submit <id> <url>", desc: "Submit work with PR link" },
    { cmd: "/bounty stats", desc: "Stats + escrow balance" },
  ]},
  { section: "Working Groups", cmds: [
    { cmd: "/groups", desc: "List all groups + member counts" },
    { cmd: "/group <name>", desc: "Group detail + members" },
    { cmd: "/group join <name>", desc: "Join a group (badge required)" },
    { cmd: "/group leave <name>", desc: "Leave a group" },
    { cmd: "/wg report <group>", desc: "File biweekly report" },
    { cmd: "/wg budget <group>", desc: "Budget status" },
    { cmd: "/wg sunset <group> <date>", desc: "Set charter expiry" },
    { cmd: "/wg renew <group> <months>", desc: "Extend charter" },
    { cmd: "/wg overdue", desc: "Groups with no report this period" },
  ]},
  { section: "Game & Stats", cmds: [
    { cmd: "/game", desc: "Dice stats + bonus XP" },
    { cmd: "/leaderboard", desc: "Top players by XP" },
    { cmd: "/stats", desc: "Platform-wide statistics" },
    { cmd: "/history", desc: "Recent proposals + outcomes" },
  ]},
  { section: "Support", cmds: [
    { cmd: "/feedback <msg>", desc: "Submit support ticket" },
...
const FAQ = [
  ...
  { q: "What are the charter votes?", a: "47 governance decisions that build the DAO from the ground up. Phase 1 (6 foundation params) → Phase 2 (20 configuration params) → Phase 3 (6 operations). Plus 10 structural decisions and 5 P3 service transitions. Each phase unlocks when the previous completes. See the full tree at /decisions." },
  ...
  { q: "What is conviction voting?", a: "CV3 — a time-weighted governance system for fund allocation. Stake XRD on proposals you believe in. Conviction grows over time (3-day half-life). When conviction exceeds the threshold (10x requested amount), the proposal auto-executes and funds are released from the shared pool. Badge tier multipliers: Member 1x, Contributor 1.5x, Builder+ 2x. All parameters are BETA and subject to charter vote." },
  { q: "How do I create a CV3 proposal?", a: "Go to the Proposals page, scroll to Conviction Voting, click 'Create Conviction Proposal'. Enter a title, description, requested XRD amount, and beneficiary address. Sign with your Radix Wallet. The CV3 watcher picks it up within 60 seconds. You can also type /cv3 create in Telegram for instructions." },
  { q: "What happens when a proposal auto-executes?", a: "When conviction reaches 10x the requested amount, the smart contract automatically transfers XRD from the shared pool to the beneficiary. No admin step needed. If the proposal is linked to a bounty, the bounty is auto-funded. All stakes are returned to stakers." },
  { q: "Can I create bounties from the dashboard?", a: "Yes! Connect your wallet, go to the Bounty Board, and click '+ Create Task'. Fill in title, reward amount, category, difficulty, and deadline. You can also create bounties via /bounty create in Telegram." },
  { q: "What are the badge tier multipliers?", a: "Your badge tier amplifies your conviction voting weight. Member (0 XP) = 1x, Contributor (100 XP) = 1.5x, Builder (500 XP) = 2x, Steward (2000 XP) = 2x, Elder (10000 XP) = 2x. Earn XP through voting, proposing, completing tasks, and playing the dice game." },
  ...
  { q: "What are working groups?", a: "Teams that organize the guild's work: Guild, DAO, Radix Infra, Business Development, Marketing. Join one with /group join <name> in Telegram or from the dashboard. Groups have leads, charter with sunset dates, budgets, and biweekly reports. Use /wg report to file, /wg sunset to set expiry, /wg overdue to check status." },
