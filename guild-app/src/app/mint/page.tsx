  ...
  if (!connected) {
    return (
      <div className="text-center py-16">
        ...
        <p className="g-text-2 text-sm">Connect your Radix wallet to mint a free membership badge.</p>
      </div>
    );
  }
  ...
  if (badge) {
    return (
      <div className="space-y-4">
        ...
        <p className="g-text-2 text-[13px]">You already have a guild badge. Earn XP by voting, proposing, and contributing.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardBody className="pt-5">
          ...
          <button onClick={handleMint} disabled={minting || !username.trim()}
            className={`g-btn mt-4 w-full py-3 text-sm ${!username.trim() || minting ? "opacity-50 cursor-not-allowed" : ""}`}>
      ...
      <Card>
        ...
        <CardBody><TierProgression /></CardBody>
      </Card>
    </div>
  );
}
