    ...
    <div className="px-4 pb-4 sm:px-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="bg-muted rounded-lg p-3 ring-1 ring-transparent hover:ring-[#4ea8de]/30 transition-all">
          ...
          <ul className="space-y-1.5 text-[11px] text-muted-foreground">
            <li className="flex items-start gap-1.5">
              <Zap className="h-3 w-3 text-[#4ea8de] mt-0.5 shrink-0" />
              <span>Free — no transaction fees</span>
            </li>
            <li className="flex items-start gap-1.5">
              <Shield className="h-3 w-3 text-[#4ea8de] mt-0.5 shrink-0" />
              <span>1 badge = 1 vote (merit-weighted)</span>
            </li>
            <li className="flex items-start gap-1.5">
              <Clock className="h-3 w-3 text-[#4ea8de] mt-0.5 shrink-0" />
              <span>Quick polls, charter params, temp checks</span>
            </li>
            <li className="flex items-start gap-1.5">
              <Star className="h-3 w-3 text-[#4ea8de] mt-0.5 shrink-0" />
              <span>+10 XP per vote + dice roll</span>
            </li>
          </ul>
        </div>
        {/* On-chain */}
        <div className="bg-muted rounded-lg p-3 ring-1 ring-transparent hover:ring-[#a78bfa]/30 transition-all">
          ...
          <ul className="space-y-1.5 text-[11px] text-muted-foreground">
            <li className="flex items-start gap-1.5">
              <Scale className="h-3 w-3 text-[#a78bfa] mt-0.5 shrink-0" />
              <span>Binding — recorded on Radix ledger</span>
            </li>
            <li className="flex items-start gap-1.5">
              <Coins className="h-3 w-3 text-[#a78bfa] mt-0.5 shrink-0" />
              <span>XRD-weighted vote power</span>
            </li>
            <li className="flex items-start gap-1.5">
              <FileText className="h-3 w-3 text-[#a78bfa] mt-0.5 shrink-0" />
              <span>Treasury decisions, formal governance</span>
            </li>
            <li className="flex items-start gap-1.5">
              <CheckCircle className="h-3 w-3 text-[#a78bfa] mt-0.5 shrink-0" />
              <span>Same system used by Radix Foundation</span>
            </li>
          </ul>
        </div>
      </div>
...
const BOUNTY_STEPS = [
  { icon: Coins, label: "Create & Fund", detail: "Admin posts bounty with XRD escrowed in Scrypto contract.", status: "open" },
  { icon: ClipboardCheck, label: "Claim", detail: "Member claims task via bot or dashboard.", status: "assigned" },
  { icon: FileText, label: "Submit", detail: "Deliverables uploaded — 72h dispute window starts.", status: "submitted" },
  { icon: CheckCircle, label: "Verify", detail: "Admin reviews quality, approves or disputes.", status: "verified" },
  { icon: Star, label: "Pay", detail: "Escrow releases XRD to worker + XP awarded.", status: "paid" },
];
...
const EARN_ACTIONS = [
  ...
  { action: "Complete bounty", xp: "+50" },
  { action: "Daily streak bonus", xp: "+5/day" },
];
            ...
            <div key={tier} className="flex items-center gap-2 group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <Badge variant="secondary" className="text-[8px] font-mono ml-auto">{weight}× vote</Badge>
                </div>
...
const CHARTER_PHASES = [
  {
    ...
    title: "Foundations",
    status: "active",
    params: ["DAO name", "Treasury split", "Quorum", "Voting period", "Badge tiers"],
    desc: "Core parameters that define the DAO's structure.",
  },
  {
    phase: 2,
    title: "Operations",
    status: "locked",
    params: ["Bounty rules", "Dispute resolution", "Role permissions", "Working groups"],
    desc: "Unlocked after Phase 1 — operational policies.",
  },
  {
    phase: 3,
    title: "Expansion",
    status: "locked",
    params: ["Federation", "Sub-DAOs", "Tokenomics", "External integrations"],
    desc: "Unlocked after Phase 2 — scaling and external integration.",
  },
];
            ...
            <div key={p.phase}>
              <button
                className={`w-full text-left rounded-lg px-3 py-3 transition-all ${
                  isActive
                    ? "bg-[#f472b6]/10 ring-1 ring-[#f472b6]/30"
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  {isActive ? (
                    <Unlock className="h-4 w-4 text-[#f472b6]" />
                  ) : (
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {p.params.map(param => (
                      <Badge
                        key={param}
                        variant="outline"
                        className={`text-[9px] font-mono ${isActive ? "" : "opacity-50"}`}
                      >
                        {param}
                      </Badge>
                    ))}
...
const ARCH_BLOCKS = [
  { icon: MessageSquare, label: "Telegram Bot", desc: "Grammy · governance · dice", color: "#4ea8de", col: 0, row: 0 },
  { icon: LayoutDashboard, label: "Dashboard", desc: "Next.js 16 · React 19", color: "#2dd4bf", col: 1, row: 0 },
  { icon: Server, label: "API / SQLite", desc: "Express · REST · DB", color: "#f59e0b", col: 0, row: 1 },
  { icon: Box, label: "Scrypto", desc: "Badges · Escrow", color: "#a78bfa", col: 1, row: 1 },
  { icon: Layers, label: "CV2", desc: "On-chain governance", color: "#f472b6", col: 2, row: 0 },
  { icon: Users, label: "Gateway API", desc: "Radix mainnet", color: "#00e49f", col: 2, row: 1 },
];
...
export function UserJourneyWidget() {
  ...
  /* Auto-play: advance step, then stage */
  useEffect(() => {
    if (!autoPlay) return;
    const maxSteps =
      activeStage === 0 ? QUICK_STEPS.length
        : activeStage === 2 ? BOUNTY_STEPS.length
          : 0;
    ...
    <Card className="overflow-hidden" >
      <CardContent className="p-0">
        <div
          ...
          tabIndex={0}
          role="tabpanel"
          aria-label="User journey walkthrough"
        ...
        >
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex gap-1">
                ...
                <Button
                  ...
                  size="icon-xs"
                  onClick={next}
                  disabled={activeStage === STAGE_META.length - 1}
          ...
          {/* Stage body */}
          <div id={`stage-panel-${stage.id}`} className="journey-fade-in" key={activeStage}>
            {renderStageBody()}
          </div>
            ...
            {STAGE_META.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === activeStage ? "w-4 bg-primary" : "w-1.5 bg-muted-foreground/30 hover:bg-muted-foreground/50"
                }`}
